...
async def test_generate_reconstruction_sync(monkeypatch):
    ...
    monkeypatch.setattr("app.crud.reconstructions.create_reconstruction", fake_create_reconstruction)

    req = AIRequest(task=AITask.reconstruction, input_text="idea", project_id="p1", dry_run=False)
    res = await ai_routes.generate_reconstruction(req, session=DummySession())
    assert "id" in res
...
async def test_generate_reconstruction_background(monkeypatch):
    ...
    bg = BackgroundTasks()
    res = await ai_routes.generate_reconstruction(req, session=DummySession(), background=True, background_tasks=bg)
